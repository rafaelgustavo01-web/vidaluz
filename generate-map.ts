...
https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0' } }, (res) => {
  ...
  res.on('end', () => {
    ...
    console.log("export const WIKI_MAP: Record<string, string> = {");
    pages.forEach(p => {
      const title = p.title.replace('File:', '').replace(' (Rider-Waite Smith tarot deck).png', '');
      const url = p.imageinfo[0].url;
      console.log(`  "${title.toLowerCase()}": "${url}",`);
    });
    console.log("};");
  });
});
